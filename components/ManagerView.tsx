...
import { Users, Activity, CheckCircle, ScrollText, ChevronRight } from "lucide-react";
      ...
      <Card className="overflow-hidden border-border">
        <div className="divide-y divide-border">
          ...
          {sessions?.map((session: { id: string; patientName: string | null; status: string; startedAt: string; messageCount: number }) => (
            <div key={session.id} className="p-4 hover:bg-muted/50 transition-colors cursor-pointer flex items-center justify-between" onClick={() => setSelectedSessionId(session.id)}>
